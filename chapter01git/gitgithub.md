...
1. 快照式管理
    ...
    > 注：这种快照式管理一方面在某种程度上会增加小文件的数量。尤其是当一个大型 Git 托管平台需要进行迁移时，其数据量固然大，但最难处理是小数据，难免在复制迁出过程中丢失一些。另一方面是对大文件的管理相对无力。如果项目中有大文件，且大文件变更也频繁，那么这种快照式管理带来的麻烦就是项目库相当庞大。在这方面 github 深有体会，并专门对大文件的托管作了研究——[github 自身采用的大文件管理][11]和 [github 开发的 git 大文件管理扩展][12]。
...
4. 三种状态
    ...
    > 2.hooks：存储钩子的文件夹
    > 3. logs：存储日志的文件夹
    > 4. refs：包含三个子文件夹 heads、remotes 和 tags，可以看到本地分支、远程分支和标签文件
    > 5. objects：存放git对象
    > 6. config：存放各种设置文档
    > 7. HEAD：指向当前所在分支的指针文件路径，一般指向refs下的某文件
    > 8. COMMIT_EDITMSG : 上次提交信息
    > 9. description : 仓库描述
    > 10. index : 暂存区域（就是上文的所谓简单的文件）
    > 11. info : 仓库的额外信息
    > 12. ORIG_HEAD : HEAD 的前一个状态
    > 13. packed-refs : 包含打包了的指针和标签

    > + 另外，可以参考[这个表][15]可以对 Git 的各个区域有更深刻的理解

## [GitHub][17]
### 简介
1. GitHub 是个托管平台，目前来说是最大的 Git 仓库托管地，同时也是数以百万计的开发者与项目协作地。
2. GitHub 除了可以托管项目外，还提供了相当多的工具，如问题跟踪、代码审阅、Pages 服务和 Gist 服务等，在一定程度上主导了开源项目的走向。
3. GitHub 同时提供付费账户和免费账户。
4. GitHub 提供企业版托管平台的下载，当然也是付费的。
